...
<body class="container py-4">
    ...
    <a href="<?php echo e(route('mediciones.pdf')); ?>" class="btn btn-primary mb-3">
    Descargar reporte PDF
</a>
 
     <form id="form-toggle-recepcion" method="POST" action="<?php echo e(route('mediciones.toggle-recepcion')); ?>">
    <?php echo csrf_field(); ?>
    <button type="submit" class="btn btn-warning mb-3">
        <?php echo e($recepcionActiva ? 'Detener recepción de datos' : 'Reanudar recepción de datos'); ?>

    </button>
</form>

    
     <form method="GET" action="<?php echo e(route('mediciones.index')); ?>" class="mb-4 d-flex align-items-end gap-2">
    <div>
        <label for="fecha" class="form-label">Filtrar por fecha:</label>
        <input type="date" name="fecha" id="fecha" class="form-control" value="<?php echo e(request('fecha')); ?>">
    </div>
    <button type="submit" class="btn btn-secondary">Filtrar</button>
    <a href="<?php echo e(route('mediciones.index')); ?>" class="btn btn-outline-secondary">Limpiar</a>
</form>
    ...
    <table class="table table-bordered" id="tabla-mediciones">
        ...
        <tbody>
            <?php $__currentLoopData = $mediciones; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $m): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <tr data-id="<?php echo e($m->id); ?>" <?php if($m->valor_ph < 6.5 || $m->valor_ph > 8.5): ?> class="table-danger" <?php endif; ?>>
                    ...
                    <td>
                        <button class="btn btn-danger btn-sm eliminar-medicion" data-id="<?php echo e($m->id); ?>">
                            Eliminar
                        </button>
                    </td>
                </tr>
    ...
    <script>
        ...
        <?php $__currentLoopData = $mediciones; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $m): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            medicionIndices[<?php echo e($m->id); ?>] = <?php echo e(count($mediciones) - $i - 1); ?>;
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>

        let labels = <?php echo json_encode(collect($mediciones)->pluck('hora')->reverse()->values(), 15, 512) ?>;
        let data = <?php echo json_encode(collect($mediciones)->pluck('valor_ph')->reverse()->values(), 15, 512) ?>;
        let medicionesIds = <?php echo json_encode(collect($mediciones)->pluck('id')->reverse()->values(), 15, 512) ?>;
...
`;
            ...
            // Actualizar el valor en tiempo real (si el elemento existe)
...
<?php /**PATH C:\Users\CESAR\Downloads\ph-monitor-laravel-main\ph-monitor-laravel-main\resources\views/mediciones/index.blade.php ENDPATH**/ ?>